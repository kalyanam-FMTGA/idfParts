...
for climate in Berlin ; do
	...
	elif  [ $climate == 'Amsterdam' ]; then weather=NLD_Amsterdam.062400_IWEC.epw
	elif  [ $climate == 'Athens' ]; then weather=GRC_Athens.167160_IWEC.epw
	elif  [ $climate == 'Berlin' ]; then weather=DEU_Berlin.103840_IWEC.epw
	elif  [ $climate == 'Madrid' ]; then weather=ESP_Madrid.082210_SWEC.epw	
fi
...
###############################
	...
	if [ -s runs/${run}/table.txt ] ; then
	FILESIZE_eout=$(stat -c%s "runs/${run}/table.txt")
		if [ $FILESIZE_eout -lt 300 ] ; then
			rm runs/${run}/eplus* 
			rm runs/${run}/table.txt 
			rm runs/${run}/disag.txt
		fi
	fi	
